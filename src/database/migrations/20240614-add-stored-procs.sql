...
DECLARE _id BIGINT;
BEGIN
    INSERT INTO tags (name, user_id) VALUES (_name, _user_id) RETURNING id INTO _id;
    RETURN _id;
END; $$ LANGUAGE plpgsql;
...
RETURNS TABLE(id BIGINT, name TEXT) AS $$
BEGIN
    RETURN QUERY SELECT tags.id, tags.name FROM tags WHERE tags.id = _id;
END; $$ LANGUAGE plpgsql;
...
RETURNS VOID AS $$
BEGIN
    DELETE FROM project_tags WHERE project_tags.tag_id = _tag_id AND project_tags.project_id = _project_id;
END; $$ LANGUAGE plpgsql;
